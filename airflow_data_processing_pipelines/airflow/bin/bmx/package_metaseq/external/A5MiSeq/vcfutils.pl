...
sub main {
	...
	my %func    = (
		...
		ucscsnp2vcf => \&ucscsnp2vcf,
		filter4vcf  => \&varFilter,
		ldstats     => \&ldstats,
		gapstats    => \&gapstats,
		splitchr    => \&splitchr,
		vcf2fq      => \&vcf2fq
...
sub subsam {
	...
	while (<$fh>) {
		if (/^##/) {
			...
			pop(@s)
			  if ( @s == 9 );    # no sample selected; remove the FORMAT field
			print join( "\t", @s ), "\n";
		}
		else {
			my @t = split;
...
sub fillac {
	die(
qq/Usage: vcfutils.pl fillac <in.vcf>\n\nNote: The GT field MUST BE present and always appear as the first field.\n/
	  )
	  if ( @ARGV == 0 && -t STDIN );
	while (<>) {
		if (/^#/) {
	...
	while (<>) {
		if (/^([^#\s]+)\s(\d+)/) {
			...
			if (/NEIR=([\d\.]+)/) {
				++$n;
				++$y, $x += $pos - $last
				  if ( $lastchr eq $chr && $pos > $last && $1 > $cutoff );
			}
			$last    = $pos;
			$lastchr = $chr;
		}
...
sub qstats {
	...
	getopts( 'r:s:v', \%opts );
	die(
		"Usage: vcfutils.pl qstats [-r ref.vcf] <in.vcf>\n
...
# Still on the same chromosome and the first element's window still affects this position?
			last
			  if ( $staging[0][3] eq $t[0]
				&& $staging[0][4] + $staging[0][2] + $max_dist >= $t[1] );
			varFilter_aux( shift(@staging), $opts{p} )
			  ;    # calling a function is a bit slower, not much
		}
		my $flt = 0;
			...
			if ( $type == 3 ) {              # an indel
				...
				for my $x (@staging) {
					next
					  if ( ( $x->[0] & 3 ) != 3
						|| $x->[4] + $x->[2] + $ow < $t[1] );
					if (   $x->[4] + length( $x->[7] ) - 1 == $t[1]
						&& substr( $x->[7], -1, 1 ) eq substr( $t[4], 0, 1 )
...
sub gapstats {
	...
	while (<>) {
		...
		my @t = split;
		next
		  if ( length( $t[3] ) == 1 && $t[4] =~ /^[A-Za-z](,[A-Za-z])*$/ )
		  ;    # not an indel
		my @s = split( ',', $t[4] );
		for my $x (@s) {
...
\n/
	...
	while (<>) {
		...
		# Grab the sequence length info from the header text
		if (/^##contig=/) {
			my @fields = ( $_ =~ m/ID=([^,]+),length=(\d+)/ );
			$seqlen->{ $fields[0] } = $fields[1];
		}
		next if (/^#/);
		...
		if ( $last_chr ne $t[0] ) {
			...
			&v2q_post_process( $last_chr, \$seq, \$qual, \@gaps, $opts{l}, $seqlen )
		...
		# a SNP or reference
		if (   length( $t[3] ) == 1 && $t[7] !~ /INDEL/ && $t[4] =~ /^([A-Za-z.])(,[A-Za-z])*$/ ) {
			my ( $ref, $alt ) = ( $t[3], $1 );
			my ( $b, $q );
			$q = $1 if ( $t[7] =~ /FQ=(-?[\d\.]+)/ );
			if ( $q < 0 ) {
...
sub v2q_post_process {
	my ( $chr, $seq, $qual, $gaps, $l, $seqlen ) = @_;
	for my $g (@$gaps) {
		my $beg = $g->[0] > $l ? $g->[0] - $l : 0;
		my $end = $g->[0] + $g->[1] + $l;
		$end = length($$seq) if ( $end > length($$seq) );
		substr( $$seq, $beg, $end - $beg ) =
		  lc( substr( $$seq, $beg, $end - $beg ) );
	}

	if (length($$seq) < $seqlen->{$chr}) {
		my $missingBases = $seqlen->{$chr} - length($$seq);
		$$seq =~ s/^(.*)/$1 . 'n' x $missingBases/mge;
		$$qual =~ s/^(.*)/$1 . '!' x $missingBases/mge;
	}
