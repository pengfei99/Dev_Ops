...
# miseq_preproc
	# /Softs/bioinfo/A5MiSeq/trimmomatic.jar
	# /Softs/bioinfo/A5MiSeq/adapter.fasta
	# Only paired end
...
miseq_preproc(){
	...
	if [[ ${9} -eq 0 ]] ; then
		...
		java -jar /Softs/bioinfo/A5MiSeq/trimmomatic.jar SE -threads ${1} -phred33 -trimlog ${OUTPUT}/Trimmomatic_${BASENAME}/trim.log ${2} ${3} ILLUMINACLIP:/Softs/bioinfo/A5MiSeq/adapter.fasta:2:30:10 SLIDINGWINDOW:4:15 LEADING:3 TRAILING:3 MINLEN:${5} 1> ${OUTPUT}/Trimmomatic_${BASENAME}/trimmomatic.out 2> ${OUTPUT}/Trimmomatic_${BASENAME}/trimmomatic.err
	else
		java -jar /Softs/bioinfo/A5MiSeq/trimmomatic.jar SE -threads ${1} -phred33 ${2} ${3} ILLUMINACLIP:/Softs/bioinfo/A5MiSeq/adapter.fasta:2:30:10 SLIDINGWINDOW:4:15 LEADING:3 TRAILING:3 MINLEN:${5} &> /dev/null
	fi
	...
	if [[ ${9} -eq 0 ]] ; then
		...
		/Softs/bioinfo/A5MiSeq/sga preprocess --quality-trim=25 --quality-filter=20 --min-length=${5} --pe-mode=0 ${2} > ${3} 2> ${OUTPUT}/SGA_Preprocess_${BASENAME}/sga_preprocess.err
	else
		/Softs/bioinfo/A5MiSeq/sga preprocess --quality-trim=25 --quality-filter=20 --min-length=${5} --pe-mode=0 ${2} > ${3} 2> /dev/null
	fi
	...
	if [[ ${10} == *"M"* ]] ; then
		...
		if [[ ${9} -eq 0 ]] ; then
			/Softs/bioinfo/A5MiSeq/sga index --threads=${1} --prefix=${OUTPUT}/SGA_Index_${BASENAME}/index --disk=${SGA_INDEX_MEMORY} --no-reverse ${2} > ${OUTPUT}/SGA_Index_${BASENAME}/index.out 2> ${OUTPUT}/SGA_Index_${BASENAME}/index.err
		else
			/Softs/bioinfo/A5MiSeq/sga index --threads=${1} --prefix=${OUTPUT}/SGA_Index_${BASENAME}/index --disk=${SGA_INDEX_MEMORY} --no-reverse ${2} &> /dev/null
		fi
		...
		if [[ ${9} -eq 0 ]] ; then
			...
			/Softs/bioinfo/A5MiSeq/sga correct --threads=${1} --prefix=${OUTPUT}/SGA_Index_${BASENAME}/index --kmer-size=${6} --kmer-threshold=${7} ${2} -o ${3} > ${OUTPUT}/SGA_Correct_${BASENAME}/sga_correct.out 2> ${OUTPUT}/SGA_Correct_${BASENAME}/sga_correct.err
		else
			/Softs/bioinfo/A5MiSeq/sga correct --threads=${1} --prefix=${OUTPUT}/SGA_Index_${BASENAME}/index --kmer-size=${6} --kmer-threshold=${7} ${2} -o ${3} &> /dev/null
		fi
...
while getopts f:r:x:y:z:s:q:l:p:m:t:n:k:e:c:a:onlyqc OPTION
do
	case ${OPTION} in
		f)
			FASTQ_FWD="${OPTARG}"
